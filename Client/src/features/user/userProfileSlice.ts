...
export const userProfileSlice = createSlice({
  ...
  reducers: {
    ...
    setUserPostsAndReposts: (
      state,
      action: PayloadAction<(Post | Repost)[]>,
    ) => {
    ...
    likePostOrRepostInUserProfile: (
      ...
      state.otherUserPostsAndReposts = state.otherUserPostsAndReposts.map(
        (item) => {
          if (item.type === ItemType.Post) {
            const post = item as Post;
            if (post._id === action.payload.postId) {
        ...
        if (item.type === ItemType.Post) {
          ...
          if (repost.post._id === action.payload.postId) {
            return {
              ...
              post: {
                ...repost.post,
                liked_by: repost.post.liked_by.filter(
                  (user) => user._id !== action.payload.user._id,
                ),
                likes: repost.post.likes - 1,
              },
    ...
    deletePostOrRepostInUserProfile: (
      state,
      action: PayloadAction<{ postId?: string; repostId?: string }>,
    ) => {
      state.userPostsAndReposts = state.userPostsAndReposts.filter((item) => {
        if (item.type === ItemType.Post) {
          ...
          const repost = item as Repost;
          return (
            repost._id !== action.payload.repostId &&
            repost.post._id !== action.payload.postId
          );
        }
      });
    },
    ...
    followUser: (state, action: PayloadAction<User>) => {
      const isFollowing = state.userProfileInfo?.followers.some(
    ...
    unfollowUser: (state, action: PayloadAction<User>) => {
      ...
      state.userProfileInfo = {
        ...state.userProfileInfo,
        followers: state.userProfileInfo.followers.filter(
          (follower) => follower._id !== action.payload._id,
        ),
        followerCount: state.userProfileInfo.followerCount - 1,
      };
    ...
    unfollowRandomUser: (
      ...
      state.users = state.users.map((randomUser) => {
        if (randomUser._id === action.payload.userTofollowId) {
          return {
            ...randomUser,
            followers: randomUser.followers.filter(
              (follower) =>
                follower._id !== action.payload.authenticatedUser._id,
            ),
          };
        }
        return randomUser;
