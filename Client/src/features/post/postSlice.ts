...
export const postSlice = createSlice({
  ...
  reducers: {
    ...
    likePost: (
      state,
      action: PayloadAction<{ postId: string; user: User }>,
    ) => {
    ...
    unlikePostInPostAndReplyPage: (
      ...
      state.post = {
        ...state.post,
        liked_by: state.post.liked_by.filter(
          (user) => user._id !== action.payload.user._id,
        ),
        likes: state.post.likes - 1,
      };
    ...
    unlikePostReply: (
      ...
      state.replies = state.replies.map((postReply) => {
        if (postReply._id === action.payload.postId) {
          return {
            ...postReply,
            liked_by: postReply.liked_by.filter(
              (user) => user._id !== action.payload.user._id,
            ),
            likes: postReply.likes - 1,
          };
    ...
    deletePostReply: (state, action: PayloadAction<string>) => {
      state.replies = state.replies.filter(
        (postReply) => postReply._id !== action.payload,
      );
      // will update the children array of the parent or root post in post and reply page
      if (state.post) {
        state.post = {
          ...state.post,
          children: state.post?.children.filter(
            (postChild) => postChild._id !== action.payload,
          ),
        };
      }
    },
    updatePostReply: (state, action: PayloadAction<Post>) => {
      state.replies = state.replies.map((postReply) =>
        postReply._id === action.payload._id ? action.payload : postReply,
      );
    },
    likeParentOfRootPostInPostAndRepliesPage: (
      ...
      if (
        state.post &&
        state.post.parent &&
        state.post.parent._id === action.payload.postId
      ) {
        state.post = {
          ...state.post,
          parent: {
    ...
    unlikeParentOfRootPostInPostAndRepliesPage: (
      ...
      if (
        ...
        state.post = {
          ...
          parent: {
            ...state.post.parent,
            liked_by: state.post.parent.liked_by.filter(
              (user) => user._id !== action.payload.user._id,
            ),
            likes: state.post.parent.likes - 1,
          },
      ...
      state.userReplies = state.userReplies.map((userReply) => {
        if (userReply.parent._id === action.payload.postId) {
          return {
            ...
            parent: {
              ...userReply.parent,
              liked_by: userReply.parent.liked_by.filter(
                (user) => user._id !== action.payload.user._id,
              ),
              likes: userReply.parent.likes - 1,
            },
    ...
    unlikeUserReplyInUserReplies: (
      ...
      state.userReplies = state.userReplies.map((userReply) => {
        if (userReply._id === action.payload.postId) {
          return {
            ...userReply,
            liked_by: userReply.liked_by.filter(
              (user) => user._id !== action.payload.user._id,
            ),
            likes: userReply.likes - 1,
          };
    ...
    deleteUserReplyInUserReplies: (state, action: PayloadAction<string>) => {
      state.userReplies = state.userReplies.filter(
        (userReply) => userReply._id !== action.payload,
      );
    },
    updateUserReplyInUserReplies: (state, action: PayloadAction<Post>) => {
      state.userReplies = state.userReplies.map((userReply) =>
        userReply._id === action.payload._id ? action.payload : userReply,
      );
    },
  },
});
