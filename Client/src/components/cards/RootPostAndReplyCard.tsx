...
const RootPostAndReplyCard: FC<IRootPostAndReplyCard> = ({
  post,
  isRootPost,
}) => {
  const { token, user: authenticatedUser } = useAppSelector(
    (state) => state.auth,
  );
  ...
  const isParentReposted = repostChecker(
    userReposts,
    post?.parent?._id ?? "",
    authenticatedUser?._id ?? "",
  );
  const isRootPostReposted = repostChecker(
    userReposts,
    post?._id ?? "",
    authenticatedUser?._id ?? "",
  );
    ...
    <div className="w-full flex-col flex items-center">
      ...
      {post?.parent && (
        <MemoizedThreadAndRepostCard
          ...
          replyingTo={
            post?.parent?.parent?.creator
              ? post.parent.parent.creator.username
              : null
          }
        />
      )}
