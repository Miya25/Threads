...
const LogInForm = (): JSX.Element => {
  ...
  const inputFieldStyle =
    "placeholder:text-[#726e6e] text-white block w-full border border-borderColor rounded-sm p-2 shadow-sm focus:outline-none focus:border-none focus:ring-1 sm:text-sm bg-transparent";
  ...
  async function handleLogIn(
    ...
    try {
      ...
      if (error.status === 401) {
        showToast(
          "The username or password you are trying to log in with are incorrect",
          true,
        );
      } else {
        console.error(error);
    ...
    <>
      ...
      >
        <h1 className="text-white font-semibold text-[1.5rem] self-start">
          <span className="font-light text-lightText">Log in to</span>{" "}
