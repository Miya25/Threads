...
const EditUserProfileModal = (): JSX.Element => {
  ...
  const { toEditUserInfo, userDefaultProfileImage } = useAppSelector(
  ...
  const [imagePreview, setImagePreview] = useState<string>(
    toEditUserInfo?.displayed_picture?.url || "",
  );
  ...
  async function handleUpdateUserInfo(
    ...
    try {
      ...
      const updatedUserInfo = await updateUserProfileMutation({
        newUserInfo: data,
        token,
      }).unwrap();
  ...
  return (
    <div className="bg-[#000000bd] backdrop-blur-sm fixed w-[100vw] h-[100vh] z-20 top-0 left-0 flex items-center justify-center">
      {/* close icon */}
      <p
        className="top-5 right-5 text-[#525151] text-[1.2rem] md:text-[1.5rem] absolute p-2 md:p-3 font-thin rounded-full bg-[#252424ce] hover:bg-[#424141ba] cursor-pointer"
      ...
      <div className="w-[90%] max-w-[600px] h-auto bg-matteBlack mt-8 md:mt-0 rounded-md border-borderColor border p-4">
        <h2 className="text-center text-white font-medium mb-4">
          Edit profile
        </h2>
        <form
          action=""
        ...
        >
            ...
            <div className="h-[60px] w-[60px] md:h-[80px] md:w-[80px] relative rounded-full ">
              <label htmlFor="file-input">
                <div className="flex opacity-0 hover:opacity-100 hover:bg-[#1715153c] w-full cursor-pointer rounded-full h-full absolute top-0 left-0 ease-in-out duration-300 items-center justify-center">
                  <p className="text-[#fff] text-[1.4rem] ml-1">
                    <FaUserPen />
                  </p>
                </div>
                <img
                  src={imagePreview ? imagePreview : userDefaultProfileImage}
                  alt="profile pic"
          ...
          <button className="bg-white hover:bg-[#e5e2e2bd] ease-in-out duration-300 py-2 rounded-md text-xs md:text-sm font-medium flex gap-1 justify-center items-center">
